...
def diet_plan_scraper(diet_plans_scrab_url):
    ...
    for food in range(0,len(foods),5):
        ...
        for day_food in range(food,food+5):
            ...
            heading = one_day_one_food.find('p').text.strip()
            foods[day_food].select('.fw-bold')[0].text.strip()
            t_meal_cal = {}
            meal_eng_data = one_day_one_food.select('.col-3 p')
            for meal_eng in range(0,len(meal_eng_data),2):
                t_meal_cal[meal_eng_data[meal_eng].text.strip()] = meal_eng_data[meal_eng+1].text.strip()
            ...
            for meal_food in range(len(meal_food_data)):
                ...
                t_meal_food_dict["name"] = meal_food_data[meal_food].select('.mb-0')[0].text.strip()
                t_meal_food_dict["quantity"] = meal_food_data[meal_food].select('.mb-0')[1].text.strip()
